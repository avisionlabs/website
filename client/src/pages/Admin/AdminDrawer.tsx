...
const EMPTY_DOWNLOADS: Downloads = {
  drivers: [], software: [], manuals: [], brochures: [], quickGuides: [], productPhotos: [],
}

// ── Form state ─────────────────────────────────────────────────────────────────

type FormState = {
  model: string; category: string; series: string; url: string
  imageUrl: string; description: string; inStock: boolean; onWebsite: boolean
  features: Feature[]
  specs: SpecRow[]
  downloads: Downloads
}
...
const EMPTY_FORM: FormState = {
  model: '', category: '', series: '', url: '', imageUrl: '', description: '',
  inStock: false, onWebsite: true,
  features: [], specs: [], downloads: EMPTY_DOWNLOADS,
}
...
const inputCls = 'w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 placeholder:text-gray-400 outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)]'
...
function FeaturesTab({ features, onChange, loading }: {
  features: Feature[]; onChange: (f: Feature[]) => void; loading: boolean
}) {
      ...
      {features.map((f, i) => (
        <div key={i} className="rounded-lg border border-gray-200 p-3">
          <div className="flex items-start gap-2">
            ...
            <DeleteBtn onClick={() => onChange(features.filter((_, idx) => idx !== i))} />
          </div>
        </div>
      ))}
      <AddBtn label="Add feature" onClick={() => onChange([...features, { title: '', description: '' }])} />
    </div>
  )
}
...
function SpecsTab({ specs, onChange, loading }: {
  specs: SpecRow[]; onChange: (s: SpecRow[]) => void; loading: boolean
}) {
  if (loading) return <SkeletonRows />
...
type DownloadSection = {
  key: keyof Downloads
  label: string
  fields: { key: string; label: string; placeholder?: string; span2?: boolean }[]
}
...
const DOWNLOAD_SECTIONS: DownloadSection[] = [
  {
    ...
    fields: [
      ...
      { key: 'os', label: 'OS', placeholder: 'Windows 7-11, macOS 10.13+', span2: true },
  ...
  {
    key: 'quickGuides', label: 'Quick Guides',
    fields: [
      { key: 'language', label: 'Language' },
      { key: 'size', label: 'Size' },
      { key: 'url', label: 'URL *', placeholder: 'https://…', span2: true },
...
function DownloadsTab({ downloads, onChange, loading }: {
  downloads: Downloads; onChange: (d: Downloads) => void; loading: boolean
}) {
  if (loading) return <SkeletonRows />

  function updateItem(section: keyof Downloads, i: number, field: string, value: string) {
  ...
  function removeItem(section: keyof Downloads, i: number) {
    onChange({ ...downloads, [section]: (downloads[section] as unknown[]).filter((_, idx) => idx !== i) })
  }
  function addItem(section: DownloadSection) {
    const blank = Object.fromEntries(section.fields.map(f => [f.key, ''])) as Record<string, string>
    onChange({ ...downloads, [section.key]: [...(downloads[section.key] as unknown[]), blank] })
  }
              ...
              {items.map((item, i) => (
                <div key={i} className="rounded-lg border border-gray-100 bg-gray-50 p-3">
                  <div className="mb-2 flex justify-end">
                    <DeleteBtn onClick={() => removeItem(section.key, i)} />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {section.fields.map(f => (
                      <div key={f.key} className={f.span2 ? 'col-span-2' : ''}>
                        ...
                        <input
                          value={item[f.key] ?? ''}
                          onChange={e => updateItem(section.key, i, f.key, e.target.value)}
...
export default function AdminDrawer({ mode, product, isOpen, onClose, onSaved, categoryOptions, seriesOptions }: Props) {
  ...
  useEffect(() => {
    ...
    if (mode === 'edit' && product) {
      ...
      Promise.all([
        fetch(apiUrl(`/api/products/${m}/features`), noCache).then(r => r.json()),
        fetch(apiUrl(`/api/products/${m}/specs`),    noCache).then(r => r.json()),
        ...
        setForm(prev => ({
          ...
          specs,
          downloads: downloads && typeof downloads === 'object' ? { ...EMPTY_DOWNLOADS, ...downloads } : EMPTY_DOWNLOADS,
        }))
      }).catch(() => {}).finally(() => setJsonbLoading(false))
    } else {
      setForm(EMPTY_FORM)
  ...
  async function handleSave() {
    ...
    const driversAndSoftware = [...form.downloads.drivers, ...form.downloads.software] as DriverItem[]
    const otherDownloads = [...form.downloads.manuals, ...form.downloads.brochures,
      ...form.downloads.quickGuides, ...form.downloads.productPhotos] as DownloadFile[]
    if (driversAndSoftware.some(d => !d.url?.trim()) || otherDownloads.some(d => !d.url?.trim())) {
  ...
  const TABS: { key: Tab; label: string }[] = [
    ...
    { key: 'features', label: form.features.length ? `Features (${form.features.length})` : 'Features' },
    { key: 'specs', label: form.specs.length ? `Specs (${form.specs.length})` : 'Specs' },
    ...
    <>
      ...
      <div className={`fixed left-0 top-0 z-[60] flex h-full w-[520px] max-w-[95vw] flex-col bg-white shadow-xl transition-transform duration-[250ms] ease-out ${
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            <div className="space-y-4">
              <FieldRow label="Model *">
                <input value={form.model} onChange={e => set('model', e.target.value)}
                  placeholder="e.g. AD-1650W" className={inputCls} />
              </FieldRow>
              ...
              <FieldRow label="Series">
                <input list="series-datalist" value={form.series}
                  onChange={e => set('series', e.target.value)}
                  placeholder="e.g. Carry-One Series" className={inputCls} />
              ...
              <FieldRow label="URL *">
                <input type="url" value={form.url} onChange={e => set('url', e.target.value)}
                  placeholder="https://www.avision.com/…" className={inputCls} />
              </FieldRow>
              ...
              <div className="flex gap-8 pt-1">
                ...
                <div className="flex items-center gap-3">
                  <Toggle value={form.onWebsite} onChange={v => set('onWebsite', v)} />
          ...
          {tab === 'features' && (
            <FeaturesTab features={form.features} onChange={v => set('features', v)} loading={jsonbLoading} />
          )}
          {tab === 'specs' && (
            <SpecsTab specs={form.specs} onChange={v => set('specs', v)} loading={jsonbLoading} />
          )}
          {tab === 'downloads' && (
            <DownloadsTab downloads={form.downloads} onChange={v => set('downloads', v)} loading={jsonbLoading} />
          )}
        </div>
        ...
        <div className="flex shrink-0 items-center justify-between border-t border-gray-200 px-6 py-4">
          ...
          {saveError && (
            <p className="mx-4 flex-1 text-center text-sm text-red-500">{saveError}</p>
          )}
          <button
            type="button"
            onClick={handleSave}
